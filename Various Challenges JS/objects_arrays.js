...
for (let i in myLoopedObj) {
  console.log(myLoopedObj[i])
}
...
// Math random example returns a random number from 0-1
console.log(Math.random());
// Math sign example eturns a 1 if a number is positive, 0 if a number is zero, and -1 if a number is negative.
console.log(Math.sign(-23));
// Math max will return the higher number
console.log(Math.max(10, 23, 4));
// Math min will return the lowest number
console.log(Math.min(100, 25, 65));
// Math pow will return the result of a to the xth power
...
// var secondHalf =
var secondHalf = sliceMe.slice(2, 5)
